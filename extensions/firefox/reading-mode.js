...
(function () {
  ...
  function startReminder() {
    ...
    storageGet("sync", { nrEyeInterval: 20 }, function (data) { reminderTimer = setInterval(showReminder, Math.max(0.01, Number(data.nrEyeInterval) || 20) * 60000); });
  }
  ...
  chrome.runtime.onMessage.addListener(function (message, sender, respond) { if (!message) return; if (message.type === "nr-reading-mode-toggle") respond({ active: toggleMode() }); else if (message.type === "nr-reading-mode-state") respond({ active: modeOpen }); });
