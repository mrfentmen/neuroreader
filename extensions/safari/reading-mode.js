...
(function () {
  ...
  var settings = { focus: false, blueLight: false, eyeRest: false, motion: false };
  ...
  function storageSet(area, values) {
    ...
    } catch (error) { /* local-only feature remains usable */ }
  }
  function normalizeSettings(value) {
    var input = value || {};
    return { focus: input.focus === true, blueLight: input.blueLight === true, eyeRest: input.eyeRest === true, motion: input.motion === true };
  }
  ...
  function isBlueLightTime(start, end) {
    var from = clockMinutes(start), to = clockMinutes(end);
  ...
  function ensureStyles() {
    ...
    style.textContent = [
      ...
      "#" + MODE_ID + " { position: fixed; inset: 0; z-index: 2147483646; overflow: auto; background: #fff; color: #111; font: 400 18px/1.75 Georgia, 'Times New Roman', serif; -webkit-font-smoothing: antialiased; }",
      "#" + MODE_ID + ".nr-mode-dark { background: #18181b; color: #f4f4f5; }",
      "#" + MODE_ID + " .nr-reading-toolbar { position: sticky; top: 0; z-index: 2; display: flex; gap: .6rem; align-items: center; padding: .75rem max(1rem, calc((100vw - 52rem) / 2)); background: inherit; border-bottom: 1px solid rgba(127,127,127,.28); font: 600 14px/1.3 system-ui, sans-serif; }",
  ...
  function updateGlobalAids() {
    ensureStyles();
    storageGet("sync", { nrSettings: settings, nrBlueStart: "19:00", nrBlueEnd: "07:00" }, function (data) {
  ...
  function findArticle() {
    ...
    for (var i = 0; i < candidates.length; i++) { var score = candidateScore(candidates[i]); if (score > bestScore) { best = candidates[i]; bestScore = score; } }
    if (best) return best;
  ...
  function transformClone(root) {
    ...
    for (var i = 0; i < nodes.length; i++) { var span = document.createElement("span"); span.setAttribute("data-nr", "1"); span.innerHTML = window.NeuroReader.transform(nodes[i].nodeValue); nodes[i].parentNode.replaceChild(span, nodes[i]); }
  }
  ...
  function recordWords(words) {
    ...
    storageGet("local", { nrReadingTotals: { date: "", words: 0 } }, function (data) {
      var today = new Date().toISOString().slice(0, 10), totals = data.nrReadingTotals || { date: today, words: 0 };
  ...
  function showReminder() {
    if (!overlay || !modeOpen || overlay.querySelector(".nr-eye-reminder")) return;
    var reminder = document.createElement("div"); reminder.className = "nr-eye-reminder";
    var close = document.createElement("button"); close.type = "button"; close.setAttribute("aria-label", "Dismiss eye rest reminder"); close.textContent = "×"; close.addEventListener("click", function () { reminder.remove(); }); reminder.appendChild(close);
    var text = document.createElement("span"); text.textContent = "Look at something 20 feet away for 20 seconds."; reminder.appendChild(text); overlay.appendChild(reminder);
  }
  function startReminder() {
    if (reminderTimer) clearInterval(reminderTimer); if (!settings.eyeRest) return;
    storageGet("sync", { nrEyeInterval: 20 }, function (data) { reminderTimer = setInterval(showReminder, Math.max(0.05, Number(data.nrEyeInterval) || 20) * 60000); });
  }
  function stopReminder() { if (reminderTimer) clearInterval(reminderTimer); reminderTimer = null; }
  function closeMode() { stopReminder(); if (overlay) overlay.remove(); overlay = null; modeOpen = false; document.documentElement.classList.remove("nr-reading-active"); updateGlobalAids(); }
  function openMode() {
    if (modeOpen) return;
    ensureStyles(); var source = findArticle(), clone = source.cloneNode(true); removeUnsafeNodes(clone); transformClone(clone);
    var words = countWords(clone.textContent); overlay = document.createElement("div"); overlay.id = MODE_ID; overlay.setAttribute("role", "dialog"); overlay.setAttribute("aria-modal", "true"); overlay.setAttribute("aria-label", "NeuroReader reading mode");
    var toolbar = document.createElement("div"); toolbar.className = "nr-reading-toolbar"; var label = document.createElement("strong"); label.textContent = "Reading mode"; toolbar.appendChild(label); var count = document.createElement("span"); count.textContent = words + " words"; toolbar.appendChild(count);
  ...
  document.addEventListener("keydown", function (event) { if (event.key === "Escape" && modeOpen) { event.preventDefault(); closeMode(); } if ((event.metaKey || event.ctrlKey) && event.shiftKey && event.key.toLowerCase() === "r") { event.preventDefault(); toggleMode(); } }, true);
  ...
  storageApi.runtime.onMessage.addListener(function (message, sender, respond) { if (!message) return; if (message.type === "nr-reading-mode-toggle") respond({ active: toggleMode() }); else if (message.type === "nr-reading-mode-state") respond({ active: modeOpen }); });
  if (storageApi.storage.onChanged && storageApi.storage.onChanged.addListener) storageApi.storage.onChanged.addListener(function (changes, area) { if (area === "sync" && (changes.nrSettings || changes.nrBlueStart || changes.nrBlueEnd || changes.nrEyeInterval)) refreshSettings(); });
  ensureStyles(); refreshSettings();
})();
