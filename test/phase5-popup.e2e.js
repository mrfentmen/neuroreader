...
(async function () {
  ...
  try {
    ...
    assert.strictEqual(await popup.locator(".pp-library-queue-toggle").textContent(), "Read later");
    await popup.click(".pp-library-queue-toggle");
    await popup.waitForFunction(() => document.querySelector(".pp-library-queue-toggle").textContent === "Remove from queue");
    await popup.waitForSelector(".pp-queue-item");
    assert.match(await popup.locator("#pp-status").textContent(), /added to your local queue/);
    await popup.click(".pp-library-queue-toggle");
    await popup.waitForFunction(() => document.querySelector(".pp-library-queue-toggle").textContent === "Read later");
    await popup.waitForSelector("#nr-queue-list .pp-color-help");
    assert.match(await popup.locator("#pp-status").textContent(), /removed from your local queue/);
    ...
    await popup.click(".pp-library-queue-toggle");
    await popup.waitForSelector(".pp-queue-item");
    await popup.click("#nr-queue-clear");
    await popup.waitForFunction(() => document.querySelector(".pp-library-queue-toggle").textContent === "Read later");
