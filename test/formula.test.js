...
ok('whitespace-only is untouched', () => assert.strictEqual(N.transform('   \n  '), '   \n  '));
...
ok('1 letter -> alternates: 1st normal, 2nd bold, 3rd normal, 4th bold', () => {
  assert.strictEqual(N.transform('a a a a'), 'a <b>a</b> a <b>a</b>');
  assert.strictEqual(N.transform('I am I'), 'I <b>a</b>m <b>I</b>');
});
ok('1-letter occurrence counter is per-word (case-insensitive)', () => {
...
ok('all punctuation is bolded', () => {
  ...
  ['<b>,</b>', '<b>!</b>', '<b>?</b>', '<b>&quot;</b>', '<b>.</b>'].forEach(p =>
    assert.ok(out.includes(p), 'missing bolded punct ' + p));
});
ok('pure punctuation token is fully bolded', () =>
  assert.strictEqual(N.transform('... \u2014 ()'), '<b>.</b><b>.</b><b>.</b> <b>\u2014</b> <b>(</b><b>)</b>'));
ok("apostrophe is punctuation and gets bolded; don't counts 4 letters", () => {
  ...
  assert.ok([2, 3].includes(boldedLetters("don't")), '4 letters -> 2 or 3 bold');
});
...
ok('roundtrip: stripping markup returns the exact original', () => {
  const samples = [
    ...
    'a', 'a b c', 'a a a', 'I', 'I I I',
    'Hyphen-ated words and -- dashes --- em-dashes',
  ];
  samples.forEach(s => assert.strictEqual(roundtrip(s), s, 'roundtrip failed for: ' + JSON.stringify(s)));
});
...
ok('HTML injection is neutralized', () => {
  ...
  assert.ok(!out.replace(/<b>|<\/b>/g, '').includes('<'), 'stray tag left behind');
});
...
{
  const sample = 'The quick brown fox jumps over the lazy dog near the river bank while the sun sets slowly behind the hills';
  ...
  ok('10k words: no crash, output non-empty', () => assert.ok(out.length > 1000));
  ok('10k words: roundtrip exact', () => assert.strictEqual(roundtrip(longText), longText));
  ok('1000 words transforms in < 100ms', () => assert.ok(ms / 10 < 100, 'too slow: ' + ms + 'ms for 10k'));
}
{
  const words = [];
