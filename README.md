...
GitHub Actions runs four jobs on every push and PR: formula, Firefox MV2 install/lint, Firefox DOM, and Chromium. GitHub Actions dependencies are pinned to reviewed commit SHAs with version comments to reduce workflow supply-chain drift. The test workflow grants its `GITHUB_TOKEN` read-only repository access; only the tag-triggered release workflow receives `contents: write` to publish a release. The Firefox lint runner installs Mozilla's toolchain in a disposable directory with the patched image parser, while the repository's normal `npm audit` stays clean. The Chromium job runs the extension, hardpage, and web-app regression suites under xvfb, so paste/transform/copy, edge cases, mobile layout, and browser behavior stay guarded.
