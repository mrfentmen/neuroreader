...
![NeuroReader brain mark](icons/neuroreader-brain.png)

NeuroReader is a small, independent project with a simple idea: reading support should be free, private, and easy to try. The brain mark represents the product's goal of helping readers find a comfortable way into a page. It is not a medical symbol and NeuroReader is not a medical product.
...
NeuroReader is a free, private reading tool for neurodivergent readers, including people with ADHD, dyslexia, autism, and other neurological differences. Paste in text or open a webpage and NeuroReader adds visual starting points to the words. The pattern changes as repeated words appear, and punctuation stays strong as a natural anchor. The goal is simple: make it easier to begin reading, stay with a passage, and finish it.

No account. No tracking. No text ever leaves your browser.

**Website:** [live demo](https://mrfentmen.github.io/neuroreader/) | **Source:** [GitHub](https://github.com/mrfentmen/neuroreader)
...
Other reading tools make different tradeoffs. OpenDyslexic focuses on a font change. Bionic Reading popularized fixed bolding patterns. NeuroReader takes another approach: a changing fixation pattern, punctuation anchors, and local processing. It is an independent project, not affiliated with either product, and the free core is available to everyone.

It is built by a neurodivergent person for people who want reading support without shame or pressure.

---

## Quick start

The app needs **no build step**. `formula.min.js` ships pre-built, so opening `index.html` just works. Development tooling is pinned to Node 20 (`.nvmrc`) and npm 10.8.2; CI reads the same Node pin and verifies the npm pin before installing dependencies so local and release checks do not drift across runtimes.
...
- Font files contain no code, just letter shapes, so they are never minified.
...
npm run test:firefox # Firefox MV2: code parity + isolated web-ext lint + real addon install
npm run lint:firefox # isolated Mozilla lint with the patched image-size parser
npm run test:security-tooling # verify isolated release tooling, parser bounds, and formula immutability
...
GitHub Actions runs four jobs on every push and PR: formula, Firefox MV2 install/lint, Firefox DOM, and Chromium. GitHub Actions dependencies are pinned to reviewed commit SHAs with version comments to reduce workflow supply-chain drift. Node and npm versions are pinned through `.nvmrc`, `package.json` engine bounds, and the package-manager declaration. CI and release installs use `npm ci --ignore-scripts`; browser binaries and system packages are installed explicitly, so dependency lifecycle hooks cannot execute unexpectedly on runners. The adaptive-pages test job and tagged Chrome release job also run `npm audit --audit-level=high` against the lockfile before browser or packaging work begins. The test workflow grants its `GITHUB_TOKEN` read-only repository access, cancels stale superseded runs, and bounds every job with an explicit timeout; only the tag-triggered release workflow receives `contents: write` to publish a release, with duplicate runs for the same tag serialized and bounded. The Firefox lint runner installs Mozilla's toolchain in a disposable directory with the patched image parser, while the repository's normal `npm audit` stays clean. The Chromium job runs the extension, hardpage, and web-app regression suites under xvfb, so paste/transform/copy, edge cases, mobile layout, and browser behavior stay guarded.
...
- `test:firefox` proves the extension layer: the shared runtime files are byte-identical to the Chromium build that passes the DOM e2e checks, the MV2 manifest passes Mozilla's own `web-ext lint` (0 errors), and the addon genuinely installs and launches in this exact Firefox binary via `web-ext run`. The linter runs in a disposable directory with a bounded local `image-size` parser; the vulnerable transitive parser never enters the repository's normal dependency tree. This command needs network access to install the temporary Mozilla toolchain; normal `npm ci` remains offline-reproducible for the app and extension tests.
...
icons/neuroreader-brain.png       Brain mark used in the README and public branding
...
tools/build-branding.js            Rebuilds the tracked brain PNG logo
...
test/phase39-site-audit.test.js       Truthful public-site audit and store-copy regression
test/phase40-popup-support.test.js    Extension popup ad/support-banner parity regression
