...
GitHub Actions runs four jobs on every push and PR: formula, Firefox MV2 install/lint, Firefox DOM, and Chromium. The Firefox lint runner installs Mozilla's toolchain in a disposable directory with the patched image parser, while the repository's normal `npm audit` stays clean. The Chromium job runs the extension, hardpage, and web-app regression suites under xvfb, so paste/transform/copy, edge cases, mobile layout, and browser behavior stay guarded.
